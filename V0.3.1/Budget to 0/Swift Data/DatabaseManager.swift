...
class DatabaseManager {
    ...
    private init() {
        ...
        openDatabase()
        createTables()
        migrateDatabase()  // ← Auto-migration!
    }
    ...
    private func migrateDatabase() {
        ...
        let query = "PRAGMA table_info(transactions);"
        ...
        var hasMonthColumn = false
        var hasTemplateIdColumn = false
        var hasDisplayOrderColumn = false
        ...
        while sqlite3_step(statement) == SQLITE_ROW {
            ...
            if columnName == "templateId" {
                hasTemplateIdColumn = true
            }
            if columnName == "displayOrder" {
        ...
        if !hasMonthColumn || !hasTemplateIdColumn || !hasDisplayOrderColumn {
    ...
    private func executeSQL(_ sql: String) {
        ...
        if sqlite3_prepare_v2(db, sql, -1, &statement, nil) == SQLITE_OK {
            if sqlite3_step(statement) == SQLITE_DONE {
                // Success - don't log for every SQL
            } else {
    ...
    func saveTemplate(_ template: BillTemplate) -> Bool {
        ...
        let insertQuery = """
            INSERT INTO bill_templates (id, title, amount, isIncome, recurrenceFrequency, category, notes, createdAt)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?);
        """
    ...
    func updateTemplate(_ template: BillTemplate) -> Bool {
        ...
        sqlite3_bind_text(statement, 7, (template.id.uuidString as NSString).utf8String, -1, nil)
    ...
    func cacheMonthBalance(month: Int, year: Int, balance: Double) {
        ...
        sqlite3_bind_double(statement, 4, Date().timeIntervalSince1970)  // Current timestamp
    ...
    // Add this function to DatabaseManager.swift
    func getEarliestTransactionDate() -> (month: Int, year: Int)? {
        // Find the earliest ACTUAL transaction (not template creation date)
        let query = """
